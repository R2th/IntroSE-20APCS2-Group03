...
## Tổng kết
- NeoVim như là  bản siêu mở rộng của Vim, sau khi cài neovim các bạn có thể mở nó bằng câu lệnh `vim` hoặc `nvim` kèm theo sau là tên file đều được.
- Cài NeoVim : `sudo apt-get install neovim `
- Các plugin của neovim đặt tại thư mục : `~/.config/nvim`
- File config : `init.vim`
- Các plugin đặt tại : `plugged/`
- Kiểm tra cài đặt với lệnh : `checkhealth`
