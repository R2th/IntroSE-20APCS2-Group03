...
Không giống như các mô hình two-stages như R-CNN, Fast-RCNN, Faster-RCNN, YOLOv1 là mô hình one-stage để nhận diện vật thể. Bạn chỉ cần cho ảnh đi qua model duy nhất một lần là có kết quả, đó là lý do vì sao có tên YOLO - You Only Look One. Mình sẽ đi trực tiếp vào các ý chính.
### Inference
...
**Non-Max Suppression**
Chú ý khi nhận diện có rất nhiều bounding boxes có thể phụ trách cho một vật thể. Để loại bỏ bớt các bounding boxes thừa chúng ta sẽ áp dụng Non-Max Suppression. Tuy nhiên trước tiên chúng ta cần biến đổi ouput một chút.
...
![](https://images.viblo.asia/64db011a-6155-4acc-8111-97201c7ca749.png)


Chúng ta sẽ có tổng cộng 98 bounding boxes. Quá trình NMS có thể được tóm tắt như hình dưới đây
![](https://images.viblo.asia/2d056b7a-639d-463c-998e-8b9829356e6a.png)


Để đơn giản gọi là $Pr(class_i) \cdot IOU_{pred}^{truth}$ là **class confidence**
* Đối với class **X** đầu tiên cho tất cả bounding boxes, nếu class confidence **X** của box nào nhỏ hơn **threshold** thì set class confidence **X**của box đó = 0
...
![](https://images.viblo.asia/e61669c7-7a78-4e9c-9bc3-78c662b89552.png)


### Kết luận
Như vậy chúng ta đã cùng tìm hiểu các ý chính trong YOLOv1. YOLOv1 có nhược điểm là chỉ predict được duy nhất một vật thể trong grid cell. Tuy nhiên các phiên bản YOLO sau đã cải thiện rất tốt vấn đề này.
Nếu thấy hay hãy upvote và chia sẻ bài viết giúp mình.

### Tài liệu tham khảo
1. https://huytranvan2010.github.io/
2. https://towardsdatascience.com/yolov1-you-only-look-once-object-detection-e1f3ffec8a89
3. https://lilianweng.github.io/lil-log/2018/12/27/object-detection-part-4.html
4. https://www.git2get.com/av/89501128.html
5. https://deepsystems.ai/
