...
1. Cấu hình AWS S3 credentials cho Elasticsearch
1. Register một repository với type S3 trên single-node Elasticsearch cũ
1. Tạo snapshot policy để tự động hóa việc chụp snapshot và đẩy lên S3 bucket từ Elasticsearch cũ
1. Register một repository tương tự trên cluster Elasticsearch mới và restore snapshot
