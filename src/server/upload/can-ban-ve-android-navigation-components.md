...
Để xác định một destimation trong Android Studio, bạn có thể tham khảo [tại đây](https://developer.android.com/topic/libraries/architecture/navigation/navigation-implementing#Identify-destinations).
...
Điều này sẽ đảm bảo bạn có tất cả các công cụ mới nhất như Navigation editor có sẵn để hỗ trợ bạn thực hiện điều hướng. Bạn có thể tìm thấy phiên bản Android Studio mới nhất [tại đây](https://developer.android.com/studio/preview/?utm_source=android-studio).
...
![](https://images.viblo.asia/d2ca70b7-ab32-46f1-8774-1f6d54ca99ce.png)

Navigation Editor được hiển thị ở trên được chia thành 3 phần khác nhau. Từ trái sang phải:
1. Phần này hiển thị danh sách tất cả các destinations trong graph và đâu là nơi navigation graph được lưu trữ.
2. Phần giữa và phần lớn là nơi bạn có thể thấy một đại diện trực quan của tất cả các destinations trong graph và các kết nối của chúng. Nếu bạn đang sử dụng namespace trong layouts của mình, data cũng sẽ được hiển thị ở đây và bạn có thể có view tốt về ứng dụng và data mẫu của mình.
3. Phần thứ ba và ngoài cùng bên phải là phần attributes. Nội dung phần này chỉ hiển thị nếu bạn chọn một destination hoặc action. Phần này sẽ hiển thị và cho phép sửa đổi các thuộc tính của destination hoặc action đã chọn.
...
![](https://images.viblo.asia/a3c80267-00f7-4ba5-8401-f08ed71ae539.png)
...
Để bắt đầu thực hiện kết nối, hãy chọn destination nơi bạn muốn bắt đầu và kéo một đường kết nối đến destination đích.
Bây giờ chúng ta sẽ có Navigation Graph trông như thế này:

![](https://images.viblo.asia/8f8c3271-f169-4fbd-93b4-9dfe86f414a5.png)
